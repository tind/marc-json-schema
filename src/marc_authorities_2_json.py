...
class marcDoc2Json:
    ...
    def __init__(self):
        ...
        self.dataDirectory = "marc_authority/"
        self.dataDirectoryFixed = "marc_authority/fixed/"
        self.schema = {}
        ...
        with open("../marc21_authority_schema.json", 'w') as aFile:
            aFile.write(
                json.dumps(
                    self.schema,
                    sort_keys=True,
    ...
    def downloadHTML(self):
        ...
        baseURL = "http://www.loc.gov/marc/authority/ad{CODE}.html"
                ...
                if value == "":
                    ...
                    for dd in BeautifulSoup(str(value)).find_all("dd"):
                        ...
                        if postionValue.find(" - ") == -1:
                            print(
                                "Error, postion is not parseable",
                                postionValue)
                ...
                for td in soup.find_all('td', {'width': '45%'}):
                    ...
                    for line in lines:
                        ...
                        if line.find("</strong>") > - \
                            ...
                            if not start.isdigit or not stop.isdigit:
                                print (
                                    "Error, the positions are not numeric",
                                    line)

                            activeCat = name

                            groups[activeCat] = (
                                {"name": name, "start": int(start), "stop": int(stop), "values": {}})
    ...
    def processHTML(self, html):
        ...
        if len(indicators) > 1:
            if indicators[0].contents[0].text.strip() == "First Indicator" and indicators[
                    1].contents[0].text.strip() == "Second Indicator":
            ...
            for aSubfield in subfields:
                ...
                for f in fields:
                    ...
                    if "$" in f and " - " in f:
                        ...
                        # this means that there were field values for this
                        # subfield
                        if f.find(" - ") > -1:
                            ...
                            foundFields[lastCode]['staticValues'][code] = {
                                "name": desc,
                                "value": code}
        ...
        # they have an entirely diffrent subfield layout using lists so process
        # that one as well if no subfields found
        ...
        if len(foundFields) == 0:
            ...
            for l in lines:
                ...
                if l.find("$") > -1 and l.find(" - ") > - \
                        1 and (l.find("(R)") > -1 or l.find("(NR)") > -1):
        ...
        if not foundSubfields:
            ...
            for f in foundFields.copy():
                ...
                if len(f) != 1:
                    ...
                    # some fields uses a range notation to list a range of
                    # possiblities, both alpha and numeric
                    if f.find("-") > -1:
                        start, stop = f.split('-')
        ...
        self.schema[fieldNumber] = {
            "repeatable": fieldRepeatable,
            "name": fieldTitle,
            "indicators": bothIndicators,
            "subfields": foundFields,
            "fixed": False}
    ...
    def processIndicators(self, indicators):
        ...
        for aIndicator in indicators:
            ...
            if len(indicatorContents) == 0:
                ...
                indicatorContents = indicatorContents.replace(
                    '<td>',
                    '').replace(
                    '</td>',
                    '').replace(
            ...
            for x in indicatorContents:
                ...
                if x.find(" - ") > -1:
                    indicatorContentsValues[
                        x.split(" - ")[0].strip()] = x.split(" - ")[1].strip().replace(
                        "<em>",
                        '').replace(
                        "</em>",
                        '')

            bothIndicators[count] = {
                'name': indicatorContentsTitle,
                'values': indicatorContentsValues}
